...
import sys
import re
from core.config import VIDEO_SOURCE, VIOLATIONS_DIR, MIN_DIST_FOR_DIRECTION, PLATES_DIR
...
from core.database import init_db, log_violation
                        ...
                        if is_wrong_direction(p_start, p_current):
                            ...
                            if not tracker.is_logged(track_id):
                                ...
                                if crop.size > 0:
                                    ...
                                    # Save specific plate crop if high confidence
                                    if plate_text and plate_conf > 0.5:
                                        plate_img_filename = f"plate_{track_id}_{plate_text.replace(' ', '_')}.jpg"
                                        cv2.imwrite(os.path.join(PLATES_DIR, plate_img_filename), crop)
                                
                                vehicle_class_name = detector.class_names[class_id]
                                log_violation(vehicle_class_name, plate_text, plate_conf, img_path)
